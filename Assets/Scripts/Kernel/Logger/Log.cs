...
namespace Kernel {
	...
	public static class Log {
		...
		/// Log日志的最大缓存数量
		/// </summary>
		private static int _LogMaxCacheNumber;
		...
		private static string strLogCacheNumber = null;	//日志缓存最大容量
		...
		//其他常量
		private const int LOG_DEFAULT_MaxCapacity = 2000; /// 日志默认最大容量
		...
		private static void SyncFileDataToLogArray() {
			if (!string.IsNullOrEmpty(_LogPath)) {
				StreamReader sr = new StreamReader(_LogPath);	//创建流读取器
				while (sr.Peek() >= 0) {
