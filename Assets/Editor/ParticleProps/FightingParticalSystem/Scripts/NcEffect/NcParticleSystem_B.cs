...
public class NcParticleSystem_B : NcEffectBehaviour_B {
	// Attribute ------------------------------------------------------------------------
	...
	void AddRenderEventCall() {
		foreach (Camera cam in Camera.allCameras) {
			NsRenderManager_B nsr = cam.GetComponent<NsRenderManager_B>();
			if (nsr == null)
				nsr = cam.gameObject.AddComponent<NsRenderManager_B>();
	...
	public float GetScaleMinMeshNormalVelocity() {
		return m_fLegacyMinMeshNormalVelocity * (m_bScaleWithTransform ? NcTransformTool_B.GetTransformScaleMeanValue(transform) : 1);
	}
	...
	public Particle[] LegacyScaleParticle(Particle[] parts, bool bScale, bool bPosUpdate) {
		...
		else fScale = 1 / NcTransformTool_B.GetTransformScaleMeanValue(transform);
	...
	public static void Ng_SetProperty(object srcObj, string fieldName, object newValue) {
		...
		if (info != null && info.CanWrite)
			...
			Debug.LogWarning(info.Name + " could not be write.");
	}
	...
	public static object Ng_GetProperty(object srcObj, string fieldName)
	{
		...
		if (info != null && info.CanRead && info.GetIndexParameters().Length == 0)
		{
			...
			Debug.LogWarning(info.Name + " could not be read.");
		return ret;
	}
